...
```bash
javac -d out/test-ui src/main/java/*.java && java -Dbaby.filePath=$(mktemp) -cp out/test-ui Baby
```
...
[
  {
    "name": "Exit",
    "aim": "Verify that the program starts, shows the greeting, accepts bye, and exits with the farewell message.",
    "command": "javac -d out/test-ui src/main/java/*.java && java -Dbaby.filePath=$(mktemp) -cp out/test-ui Baby",
    "input": [
      "bye"
    ],
